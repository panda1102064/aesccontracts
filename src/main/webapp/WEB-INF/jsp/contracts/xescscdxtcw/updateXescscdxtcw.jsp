...
<jsp:include page="../../headpart.jsp"></jsp:include>
		<div id="wrap">
		<form action="updateXescscdxtcw" method="post">
			...
			<p>&nbsp;&nbsp;一、甲方向乙方提供地下车库<input required="required" name="offer" class="dtkz" type="text" value="${aesc.offer}" />，该车位用于<input required="required" class="dtkzNum" type="text" name="purpose" value="${aesc.purpose}" />停放，车牌号为渝<input required="required" class="dtkzNum" type="text" name="plate"  value="${aesc.plate}" /> ，该停车区位于汽博新二手车市场车库内。</p>
			<p>&nbsp;&nbsp;二、租赁期限及场地使用服务费：自<input required="required" name="year_start" class="dtkzNum" type="text"  value="${aesc.year_start}" />年<input required="required" class="dtkzNum" type="text" name="month_start" value="${aesc.month_start}" />月 <input required="required" class="dtkzNum" type="text" name="day_start"  value="${aesc.day_start}" />日至 <input required="required" class="dtkzNum" type="text" name="year_end" value="${aesc.year_end}" />年<input required="required" class="dtkzNum" type="text" name="month_end" value="${aesc.month_end}" />月 <input  value="31"required="required" class="dtkzNum" type="text" name="day_end"  value="${aesc.day_end}" />日止。服务费为：<input required="required" type="text" class="dtkzNum" name="service_charge"  value="${aesc.service_charge}" />元/个/月，共计 <input required="required" type="text" class="dtkzNum" name="total"  value="${aesc.total}" />元/月（大写： <input required="required" class="dtkz" name="numeral_sed" type="text"  value="${aesc.numeral_sed}" /> ）。</p>
			<p>&nbsp;&nbsp;三、计租时间及相关费用计算时间：从<input required="required" name="year_rd" class="dtkzNum" type="text"  value="${aesc.year_rd}" />年 <input required="required" class="dtkzNum" type="text" name="month_rd" value="${aesc.month_rd}" />月 <input required="required" class="dtkzNum" type="text"  name="day_rd" value="${aesc.day_rd}" />日起算。</p>
			<p>&nbsp;&nbsp;四、乙方应向甲方支付履约保证金：￥<input required="required" name="cash_deposit" class="dtkzNum" type="text"  value="${aesc.cash_deposit}" />元（大写）<input required="required" class="dtkz" type="text" name="numeral_four"  value="${aesc.numeral_four}" />元整 ，在签定合同后由乙方一次性付清。合同有效期内若乙方擅自解除合同或有其它违反合同行为的，履约保证金和所剩租金不予退还，并纳入违约金范围之内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，只退本不退息。</p>
			<p>&nbsp;&nbsp;五、乙方按 <input required="required" name="payment_way" class="dtkz" type="text" value="${aesc.payment_way}" />向甲方交付场地租金，交款时间为每<input required="required" type="text" class="dtkz" name="payment_time" value="${aesc.payment_time}" />初第一个月的15日前 缴纳本 <input required="required" type="text" class="dtkz" name="rent" value="${aesc.rent}" />的场地租金。乙方每逾期一日，甲方有权将按照应交款总额的0.5%收取乙方的滞纳金，逾期超过15日，甲方有权单方面解除合同，并向乙方追缴所欠租金和滞纳金。</p>

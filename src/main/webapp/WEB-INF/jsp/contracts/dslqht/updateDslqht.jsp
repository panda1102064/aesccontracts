...
<%-- <jsp:include page="../../headpart.jsp"></jsp:include> --%>
		<div id="wrap">
			<form action="updateDslqht" method="post">
				...
				<p>&nbsp;&nbsp;2、乙方作为入驻汽博中心<input class="dtkz" name="rental_location" required="required" value="${aesc.rental_location}" />的承租人，承诺自愿接受甲方与重庆博展物业管理有限公司（以下简称“物业公司”）签订的《物业管理委托服务合同》之内容的约束，并按照该合同的约定向物业公司履行相关义务（包括但不限于以物业使用人的身份缴纳物管费、水电费等）。在租赁期间，若物业公司不再担任汽博中心的物业服务企业， 则乙方应当向实际担任汽博中心的物业服务企业缴纳相关费用。 </p>
				...
				<div id="main">
					<ol>
					<c:forEach items="${requestScope.dslqList}" var="dslqDate">
						<li><p><input type="hidden" name="date_id" value="${dslqDate.date_id}"/><input required="required" class="dtkzNum" name="year_str" value="${dslqDate.year_str}" />年<input required="required" class="dtkzNum" name="month_str" value="${dslqDate.month_str}" />月<input required="required" class="dtkzNum" name="day_str" value="${dslqDate.day_str}" />日至<input required="required" class="dtkzNum" name="year_ed" value="${dslqDate.year_ed}" />年<input required="required" class="dtkzNum" name="month_ed" value="${dslqDate.month_ed}" />月<input required="required" class="dtkzNum" name="day_ed" value="${dslqDate.day_ed}" />日，租金单价为：<input required="required" class="dtkzNum" name="price" value="${dslqDate.price}" />元/㎡/月，每月租金合计为：<input required="required" class="dtkzNum" name="total" value="${dslqDate.total}" />元(大写)<input required="required" class="dtkz" name="upper1" value="${dslqDate.upper1}" />元整；</p><!-- <input required="required" class="btn-danger" type="button" value="添加" id="bt" /> --></li>
					</c:forEach>	
					</ol>
				</div>
				<p>&nbsp;&nbsp;7.2、房屋租金按<input required="required" class="dtkz" name="payment1" value="季度"value="${aesc.payment1}" />支付。乙方应自本合同签订当日向甲方支付<input required="required" class="dtkz" name="payment2" value="${aesc.payment2}" />的租金，以后乙方应在每<input required="required" class="dtkz" name="payment3" value="${aesc.payment3}" />初第一个月第15日前支付本<input required="required" class="dtkz" name="payment4" value="${aesc.payment4}" />的租金。乙方逾期交纳租金，逾期在15内的，每逾期一日，应按未交纳租金的总额0.5％的标准向甲方支付滞纳金；逾期时间超过15日的，甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
				...
				<p>&nbsp;&nbsp;8、乙方应向甲方支付履约保证金：￥<input required="required" class="dtkzNum" name="cash_deposit" value="${aesc.cash_deposit}" />元(大写)<input required="required" class="dtkz" name="upper2" value="${aesc.upper2}" />元整，在本合同签订时乙方应当立即以一次性付清的方式向甲方支付。合同有效期内若乙方擅自解除合同或有其它违反合同约定行为的，履约保证金和所剩租金不予退还，并纳入违约金范围之内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，但不计算利息。</p>
				...
				<p>&nbsp;&nbsp;12、乙方租用甲方的房屋（场地）仅作为<input required="required" class="dtkz" name="purpose" value="${aesc.purpose}" />展示使用。乙方不得超出约定范围经营；如需变更经营范围，须提前二十天以书面形式告知甲方，经甲方书面同意后方可变更。否则，甲方有权单方解除合同，且不退还乙方已缴纳的租金及履约保证金。</p>

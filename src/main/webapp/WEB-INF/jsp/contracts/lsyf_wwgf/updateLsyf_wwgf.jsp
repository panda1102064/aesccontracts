...
<%-- <jsp:include page="../../headpart.jsp"></jsp:include> --%>
		<div id="wrap">
			<form action="updateLsyf_wwgf" method="post">
				...
				<p>&nbsp;&nbsp;3、租金为每<input required="required" class="dtkzNum" name="payment1" value="${aesc.payment1}" />支付，在签定合同时，<!--和第五条对换了一下，数据库有点差异 ====== 首次支付季度年的租金。乙方应向甲方支付履约保证金：￥<input required="required" class="dtkzNum" name="cash" />元， -->乙方应向甲方支付第一个<input required="required" class="dtkzNum" name="payment2" value="${aesc.payment2}" />的租金，以后乙方应在每<input required="required" class="dtkzNum" name="payment3" value="${aesc.payment3}" />第一个月15日前支付下个<input required="required" class="dtkzNum" name="payment4" value="${aesc.payment4}" />的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过15日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
				<p>&nbsp;&nbsp;4、付款方式：现金或转帐支票。</p>

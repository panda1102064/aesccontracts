...
<div id="wrap">
		<form action="updateDscwswtcw" method="post">
			...
			<br /><p><h1 style="color: #3fc712;text-align: center;">${msg}</h1></p><br />
			<p>出租方[以下简称甲方]:<u>重庆汽博实业有限公司</u></p><br />
			<p>承租方[以下简称乙方]:<input required="required" class="dtkz" name="merchant_name" value="${aesc.merchant_name}" /></p><br />
			<p>甲乙双方本着平等自愿，协商一致原则，就乙方租赁甲方汽博大厦地上停车位一事达成如下协议：</p>
			<p>一、甲方向乙方提供停车位<input required="required" class="dtkzNum" name="carport_number" value="${aesc.carport_number}" />个。该车位用于<input required="required" class="dtkz" name="purpose" value="${aesc.purpose}" />停放。车牌号为 <input required="required" class="dtkzNum" name="plate" value="${aesc.plate}" />，该停车区位于汽博大厦室外停车场内。</p>
			<p>二、租赁期限及场地使用服务费：自<input required="required" class="dtkzNum" name="year_start" value="${aesc.year_start}" />年<input required="required" class="dtkzNum" name="month_start" value="${aesc.month_start}" />月<input required="required" class="dtkzNum" name="day_start" value="${aesc.day_start}" />日至<input required="required" class="dtkzNum" name="year_end" value="${aesc.year_end}" />年<input required="required" class="dtkzNum" name="month_end" value="${aesc.month_end}" />月<input required="required" class="dtkzNum" name="day_end" value="${aesc.day_end}" />日止。服务费：<input required="required" class="dtkzNum" name="service_charge" value="${aesc.service_charge}" />元/个/月，共计<input required="required" class="dtkzNum" name="total" value="${aesc.total}" />元/月（大写：<input required="required" class="dtkz" name="upper" value="${aesc.upper}" />元整）。</p>
			<p>三、乙方须按<input required="required" class="dtkz" name="payment" value="${aesc.payment}" />向甲方交付车辆停放场地使用服务费，甲方应提供相关正规发票，交款时间为每<input required="required" class="dtkz" name="pay_time" value="${aesc.pay_time}" />初第一个月15日前缴纳下季度的服务费。乙方每逾期一日，甲方有权将按照应交款总额的0.5%收取乙方的滞纳金。</p>
			...
			<p>六、协议期满，乙方若需继续使用，应在协议到期前<input required="required" class="dtkz" name="expire" value="${aesc.expire}" />到汽博中心市场招商运营部办理续租手续；否则，将视为乙方放弃优先续租权，协议自动终止后，甲方有权将该车位转租他人，且不需对乙方另行通知。</p>
			...
			<p>十四、备注：<input class="dtkz" name="remark" value="${aesc.remark}" /></p>

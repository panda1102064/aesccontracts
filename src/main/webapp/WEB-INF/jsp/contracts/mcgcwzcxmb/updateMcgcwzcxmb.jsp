...
<%-- <jsp:include page="../../headpart.jsp"></jsp:include> --%>
        <div id="wrap">
            <form action="updateMcgcwzcxmb" method="post">
                ...
                <p>&nbsp;&nbsp;一、租赁物业基本情况</p>
                <p>&nbsp;&nbsp;1、乙方租用甲方名车广场（外展场）<input required="required" class="dtkzNum" name="area1"  value="${aesc.area1}"/>个车位，编号为<input required="required" class="dtkzNum" name="serial"  value="${aesc.serial}"/>，单价：￥<input required="required" class="dtkzNum" name="price"  value="${aesc.price}"/>元/个/月（人民币），合租办公室：<input required="required" class="dtkzNum" name="office_rent" value="${aesc.office_rent}"/>元/个/月，总计：￥<input required="required" class="dtkzNum" name="total" value="${aesc.total}"/>元/月（人民币）；</p>
                <p>&nbsp;&nbsp;二、   租赁期限</p>
                <p>&nbsp;&nbsp;上述车位租赁期限原则上为<input required="required" class="dtkzNum" name="expires" value="${aesc.expires}"/>年，从<input required="required" class="dtkzNum" name="year_start" value="${aesc.year_start}"/>年<input required="required" class="dtkzNum" name="month_start" value="${aesc.month_start}"/>月<input required="required" class="dtkzNum" name="day_start" value="${aesc.day_start}"/>日起至<input required="required" class="dtkzNum" name="year_end" value="${aesc.year_end}"/>年<input required="required" class="dtkzNum" name="month_end" value="${aesc.month_end}"/>月<input required="required" class="dtkzNum" name="day_end"  value="${aesc.day_end}" />日止。</p>
                <p>&nbsp;&nbsp;三、租金及物管费的交付方式</p>
                <p>&nbsp;&nbsp;1、租金为每<input required="required" class="dtkzNum" name="payment1" value="${aesc.payment1}" />支付，在签定合同时，乙方应向甲方支付第一个<input required="required" class="dtkzNum" name="payment2" value="${aesc.payment2}" />的租金，以后乙方应在每<input required="required" class="dtkzNum" name="payment3" value="${aesc.payment3}" />第一个月10日前支付下个<input required="required" class="dtkzNum" name="payment4" value="${aesc.payment4}" />的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过15日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
                <p>&nbsp;&nbsp;2、乙方应向甲方支付履约保证金：￥<input required="required" class="dtkzNum" name="cash_deposit" value="${aesc.cash_deposit}" />元，在本合同签订时乙方应当立即以一次性付清的方式向甲方支付。合同有效期内若乙方擅自解除合同或有其他违反合同行为的，甲方有权力单方面终止合同，履约保证金和所剩租金不予退还，并纳入违约金范围内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，只退本不退息。</p>
                <p>&nbsp;&nbsp;3、付款方式：现金、转帐支票或网上银行。</p>
                <p>&nbsp;&nbsp;3.1、上述收费起算时间：从<input required="required" class="dtkzNum" name="charge_year1" value="${aesc.charge_year1}" />年<input required="required" class="dtkzNum" name="charge_month1" value="${aesc.charge_month1}" />月<input required="required" class="dtkzNum" name="charge_day1" value="${aesc.charge_day1}" />日起算。</p>
                <p>&nbsp;&nbsp;4、物业管理服务费（按乙方承租的物业实际建筑面积计收），汽博中心<input required="required" class="dtkz" name="purpose" value="${aesc.purpose}" />的用房为商业性质的物业，物业管理服务费的收费标准经重庆市物局审批为：  <input required="required" class="dtkzNum" name="management_fee" value="${aesc.management_fee}" />元/个/月，物业管理服务费合计：<input required="required" class="dtkzNum" name="management_total" value="${aesc.management_total}" />元/月。</p>
                ...
                <p>&nbsp;&nbsp;6.1、上述收费起算时间：从<input required="required" class="dtkzNum" name="charge_year2" value="${aesc.charge_year2}" />年<input required="required" class="dtkzNum" name="charge_month2" value="${aesc.charge_month2}" />月<input required="required" class="dtkzNum" name="charge_day2" value="${aesc.charge_day2}" />日起算。</p>
                <p>&nbsp;&nbsp;四、乙方租用甲方名车广场（外展场）用于<input required="required" class="dtkz" name="syfs" value="${aesc.syfs}" />停放。乙方租赁的上述场地，仅供自己使用，不得擅自转租、转让、许可第三方使用或擅自改变合同所有约定的停放车型。如甲方发现乙方有上诉违约行为，甲方有权单方面解除合同，且不退还乙方已缴纳的租金。</p>

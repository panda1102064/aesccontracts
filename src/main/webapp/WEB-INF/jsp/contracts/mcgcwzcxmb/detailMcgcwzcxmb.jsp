...
<body>
		...
		<div class="PageNext"><jsp:include page="../../cover.jsp"/><!--动态包含--></div>
				...
				<p>&nbsp;&nbsp;一、租赁物业基本情况</p>
				<p>&nbsp;&nbsp;1、乙方租用甲方名车广场（外展场）<u> ${aesc.area1} </u>个车位，编号为<u> ${aesc.serial} </u>，单价：￥<u> ${aesc.price} </u>元/个/月（人民币），合租办公室：<u> ${aesc.office_rent} </u>元/个/月，总计：￥<u> ${aesc.total} </u>元/月（人民币）；</p>
				<p>&nbsp;&nbsp;二、	租赁期限</p>
				<p>&nbsp;&nbsp;上述车位租赁期限原则上为<u> ${aesc.expires} </u>年，从<u> ${aesc.year_start} </u>年<u> ${aesc.month_start} </u>月<u> ${aesc.day_start} </u>日起至<u> ${aesc.year_end} </u>年<u> ${aesc.month_end} </u>月<u> ${aesc.day_end} </u>日止。</p>
				<p>&nbsp;&nbsp;三、租金及物管费的交付方式</p>
				<p>&nbsp;&nbsp;1、租金为每<u> ${aesc.payment1} </u>支付，在签定合同时，乙方应向甲方支付第一个<u> ${aesc.payment2} </u>的租金，以后乙方应在每<u> ${aesc.payment3} </u>第一个月10日前支付下个<u> ${aesc.payment4} </u>的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过15日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
				<p>&nbsp;&nbsp;2、乙方应向甲方支付履约保证金：￥<u> ${aesc.cash_deposit} </u>元，在本合同签订时乙方应当立即以一次性付清的方式向甲方支付。合同有效期内若乙方擅自解除合同或有其他违反合同行为的，甲方有权力单方面终止合同，履约保证金和所剩租金不予退还，并纳入违约金范围内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，只退本不退息。</p>
				<p>&nbsp;&nbsp;3、付款方式：现金或转账。</p>
				<p>&nbsp;&nbsp;3.1、上述收费起算时间：从<u> ${aesc.charge_year1} </u>年<u> ${aesc.charge_month1} </u>月<u> ${aesc.charge_day1} </u>日起算。</p>
				<p>&nbsp;&nbsp;4、物业管理服务费（按乙方承租的物业实际建筑面积计收），汽博中心<u> ${aesc.purpose} </u>的用房为商业性质的物业，物业管理服务费的收费标准经重庆市物局审批为：  <u> ${aesc.management_fee} </u>元/个/月，物业管理服务费合计：<u> ${aesc.management_total} </u>元/月。</p>
				...
				<p>&nbsp;&nbsp;6、付款方式：现金或转账。</p>
				<p>&nbsp;&nbsp;6.1、上述收费起算时间：从<u> ${aesc.charge_year2} </u>年<u> ${aesc.charge_month2} </u>月<u> ${aesc.charge_day2} </u>日起算。</p>
				<p>&nbsp;&nbsp;四、乙方租用甲方名车广场（外展场）用于<u> ${aesc.syfs} </u>停放。乙方租赁的上述场地，仅供自己使用，不得擅自转租、转让、许可第三方使用或擅自改变合同所有约定的停放车型。如甲方发现乙方有上诉违约行为，甲方有权单方面解除合同，且不退还乙方已缴纳的租金。</p>

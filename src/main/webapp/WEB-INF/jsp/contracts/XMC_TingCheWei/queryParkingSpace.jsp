...
<c:forEach items="${requestScope.contractsList}" var="contractsList" varStatus="v">
	<tr>
		...
		<td>
			<a class="btn btn-success " onclick="window.open('<%=path%>/queryParkingSpaceById?contract_id=${contractsList.contract_id}','_blank')">详情</a>
			<a class="btn btn-danger " onclick="window.open('<%=path%>/updateParkingSpaceById?contract_id=${contractsList.contract_id}','_blank',width=600,height=500,resizable='no',scrollbars='no')">修改</a>
		</td>
	</tr>
</c:forEach>

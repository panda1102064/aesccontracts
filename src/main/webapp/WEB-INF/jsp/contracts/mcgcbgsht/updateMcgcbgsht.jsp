...
<%-- <jsp:include page="../../headpart.jsp"></jsp:include> --%>
		<div id="wrap">
			<form action="updateMcgcbgsht" method="post">
				...
				<p>&nbsp;&nbsp;2、乙方作为入驻汽博中心<input  required="required" class="dtkz" name="rental_location" value="${aesc.rental_location}" />的承租人，承诺自愿接受甲方于<input  required="required" class="dtkzNum" name="accept_year" value="${aesc.accept_year}" />年<input required="required"  class="dtkzNum" name="accept_month" value="1" value="${aesc.accept_month}" /> 月 <input required="required"  class="dtkzNum" name="accept_day" value="${aesc.accept_day}" /> 日与重庆博展物业管理有限公司（以下简称“物业公司”）签订的《物业管理委托服务合同》之内容的约束，并按照该合同的约定向物业公司履行相关义务（包括但不限于以物业使用人的身份缴纳物管费、水电费等）。在租赁期间，若物业公司不再担任汽博中心的物业服务企业， 则乙方应当向实际担任汽博中心的物业服务企业缴纳相关费用。 </p>
				...
				<p>&nbsp;&nbsp;7.1、房屋（场地）租金（按乙方实际承租的建筑面积计收）为每月租金合计人民币：<input required="required" class="dtkzNum" name="total" value="${aesc.total}" />元(大写)<input required="required" class="dtkz" name="upper1" value="${aesc.upper1}" />元整 。</p>
				<p>&nbsp;&nbsp;7.2、房屋租金按<input required="required" class="dtkz" name="payment1" value="${aesc.payment1}" />支付。乙方应自本合同签订当日向甲方支付第一个<input required="required" class="dtkz" name="payment2" value="${aesc.payment2}" />的租金，以后乙方应在每<input required="required" class="dtkz" name="payment3" value="${aesc.payment3}" />初第一个月第10日前支付本<input required="required" class="dtkz" name="payment4" value="${aesc.payment4}" />的租金。乙方逾期交纳租金，逾期在15内的，每逾期一日，应按未交纳租金的总额0.5％的标准向甲方支付滞纳金；逾期时间超过15日的，甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
				...
				<p>&nbsp;&nbsp;9.5、前述物业费用按<input required="required" class="dtkz" name="payment5" value="${aesc.payment5}" />支付；在签定合同时，乙方应向物业公司支付第一个<input required="required" class="dtkz" name="payment6" value="${aesc.payment6}" />的物业费，以后乙方应在每<input required="required" class="dtkz" name="payment7" value="${aesc.payment7}" />初25日前向物业公司支付下个<input required="required" class="dtkz" name="payment8" value="${aesc.payment8}" />的物业费。 </p>

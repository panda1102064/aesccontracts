...
<div id="wrap">
			...
			<p>承租方[以下简称乙方]:<u> ${aesc.merchant_name} </u></p><br />
			<p>甲乙双方本着平等自愿，协商一致原则，就乙方租赁甲方汽博大厦地下停车库停车位一事达成如下协议：</p>
			<p>一、甲方向乙方提供停车位<u> ${aesc.carport_number} </u>个。该车位用于<u> ${aesc.purpose} </u>停放。车牌号为 <u> ${aesc.plate} </u>，该停车区位于汽博大厦停车场内。</p>
			<p>二、租赁期限及场地使用服务费：自<u> ${aesc.year_start} </u>年<u> ${aesc.month_start} </u>月<u> ${aesc.day_start} </u>日至<u> ${aesc.year_end} </u>年<u> ${aesc.month_end} </u>月<u> ${aesc.day_end} </u>日止。服务费：<u> ${aesc.service_charge} </u>元/个/月，共计<u> ${aesc.total} </u>元/月（大写：<u> ${aesc.upper} </u>元整）。</p>
			<p>三、乙方须按<u> ${aesc.payment} </u>向甲方交付车辆停放场地使用服务费，甲方应提供相关正规发票，交款时间为每<u> ${aesc.pay_time} </u>初第一个月15日前缴纳下季度的服务费。乙方每逾期一日，甲方有权将按照应交款总额的0.5%收取乙方的滞纳金。</p>
			...
			<p>六、协议期满，乙方若需继续使用，应在协议到期前<u> ${aesc.expire} </u>到汽博中心市场招商运营部办理续租手续；否则，将视为乙方放弃优先续租权，协议自动终止后，甲方有权将该车位转租他人，且不需对乙方另行通知。</p>

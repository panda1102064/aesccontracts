...
<%-- <jsp:include page="../../headpart.jsp"></jsp:include> --%>
		<div id="wrap">
			<form action="updateQpyqkfmb" method="post">
				...
				<p>&nbsp;&nbsp;2、乙方作为入驻汽博中心<input required="required" class="dtkz" name="rental_location" value="${aesc.rental_location}" />的承租人，承诺自愿接受甲方与重庆博展物业管理有限公司（以下简称“物业公司”）签订的《物业管理委托服务合同》之内容的约束，并按照该合同的约定向物业公司履行相关义务（包括但不限于以物业使用人的身份缴纳物管费、水电费等）。在租赁期间，若物业公司不再担任汽博中心的物业服务企业， 则乙方应当向实际担任汽博中心的物业服务企业缴纳相关费用。 </p>
				<p class="zc">第二章 租赁物业概况</p>
				<p>&nbsp;&nbsp;3、租赁物业基本情况：</p>
				<p>&nbsp;&nbsp;3.1、乙方承租的甲方商业用房（场地），位于：重庆北部新区经开园金渝大道99号（以下简称“汽博中心”）内的<input required="required" class="dtkz" name="lessee_location" value="${aesc.lessee_location}" />，其建筑面积为：<input required="required" class="dtkzNum" name="building_area" value="${aesc.building_area}" />㎡ ，该租赁场地的房屋（场地）租金、物业管理服务费或其它根据场地面积计算的款项均以本款约定的租赁建筑面积计算。</p>
				...
				<p>&nbsp;&nbsp;5、甲方于<input required="required" class="dtkzNum" name="transfer_year" value="${aesc.transfer_year}" />年<input required="required" class="dtkzNum" name="transfer_month" value="${aesc.transfer_month}" />月<input required="required" class="dtkzNum" name="transfer_day" value="${aesc.transfer_day}" />日前将本合同项下的商业用房（场地）移交给乙方使用。若乙方逾期未接收的，视为甲方移交义务完成，同时，开始计收租金。</p>
				...
				<p>&nbsp;&nbsp;7.1、房屋（场地）租金（按乙方实际承租的建筑面积计收）为：<input required="required" class="dtkzNum" name="rent1" value="${aesc.rent1}" />元/月/㎡，每月租金合计人民币：<input required="required" class="dtkzNum" name="total" value="${aesc.total}" />元(大写)<input required="required" class="dtkz" name="upper1" value="${aesc.upper1}" />元整 。</p>
				...
				<p>&nbsp;&nbsp;7.4、上述收费起算时间：从<input required="required" class="dtkzNum" name="charge_year1" value="${aesc.charge_year1}" />年<input required="required" class="dtkzNum" name="charge_month1" value="${aesc.charge_month1}" />月<input required="required" class="dtkzNum" name="charge_day1" value="${aesc.charge_day1}" />日起开始计租。</p>
				...
				<p>&nbsp;&nbsp;9.1、物业管理服务费（按乙方承租的物业实际建筑面积计收），汽博中心<input required="required" class="dtkz" name="area3" value="${aesc.area3}" />的用房为商业性质的物业，物业管理服务费的收费标准经重庆市物局审批为：<input required="required" class="dtkzNum" name="management_fee1" value="${aesc.management_fee1}" />元/月/㎡，物业管理服务费：<input required="required" class="dtkzNum" name="management_fee2" value="${aesc.management_fee2}" />元/月。生产、经营垃圾转运清运费：<input required="required" class="dtkzNum" name="sanitation" value="${aesc.sanitation}" />元/m²/月。</p>
				<p>&nbsp;&nbsp;9.2、中央空调使用耗能费：按实际公示标准收费。</p>
				<p>&nbsp;&nbsp;9.3、商业用户水电交费单价分别为：电<input required="required" readonly="readonly" class="dtkzNum" name="power_rate" value="${aesc.power_rate}" />元人民币/KW/小时，水<input required="required" class="dtkzNum" name="water_rate" value="${aesc.water_rate}" />元人民币/T（含公共区域分摊费）。</p>
				<p>&nbsp;&nbsp;9.4、公共区域的水电等公摊费用按据实分摊的原则收取。</p>
				<p>&nbsp;&nbsp;9.5、前述物业费用按<input required="required" class="dtkz" name="payment5" value="${aesc.payment5}" />支付：在签定合同时，乙方应向物业公司支付<input required="required" class="dtkz" name="payment6" value="${aesc. payment6}" />的物业费，以后乙方应在每<input required="required" class="dtkz" name="payment7" value="${aesc.payment7}" />初15日前向物业公司支付<input required="required" class="dtkz" name="payment8" value="${aesc.payment8}" />的物业费。  </p>
				...
				<p>&nbsp;&nbsp;10.1、上述收费起算时间：从<input required="required" class="dtkzNum" name="charge_year2" value="${aesc.charge_year2}" />年<input required="required" class="dtkzNum" name="charge_month2" value="${aesc.charge_month2}" />月<input required="required" class="dtkzNum" name="charge_day2" value="${aesc.charge_day2}" />日起开始计租。</p>
				<p class="zc">第五章 营业资格及乙方经营范围</p>
				<p>&nbsp;&nbsp;11、乙方应具备在中华人民共和国境内依法取得经营主体资格并符合在汽博中心开业的条件。</p>
				<p>&nbsp;&nbsp;12、乙方租用甲方的房屋（场地）仅作为<input required="required" class="dtkz" name="purpose" value="${aesc.purpose}" />使用。乙方不得超出约定范围经营；如需变更经营范围，须提前二十天以书面形式告知甲方，经甲方书面同意后方可变更。否则，甲方有权单方解除合同，且不退还乙方已缴纳的租金及履约保证金。</p>
			...
			<p> <input class="dtkzNum" name="confirmation_year" value="${aesc.confirmation_year}" /> 年 <input class="dtkzNum" name="confirmation_month" value="${aesc.confirmation_month}" /> 月<input class="dtkzNum" name="confirmation_day" value="${aesc.confirmation_month}" /> 日</p>
			</div>

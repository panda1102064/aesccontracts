...
<title>Insert title here</title>
	...
	<div class="PageNext"><jsp:include page="../../cover.jsp"/><!--动态包含--></div> 
		<div id="wrap">
				...
				<ol>
				<c:forEach items="${requestScope.exhibitionHallList}" var="aescDate">
						<li><p><u> ${aescDate.year_str} </u>年<u> ${aescDate.month_str} </u>月<u> ${aescDate.day_str} </u>日至<u> ${aescDate.year_ed} </u>年<u> ${aescDate.month_ed} </u>月<u> ${aescDate.day_ed} </u>日，租金单价为：<u> ${aescDate.price} </u>元/㎡/月，每月租金合计为：<u> ${aescDate.total} </u>元；大写：<u> ${aescDate.upper} </u>元整。</p></li>
				</c:forEach>
				</ol>
				<p>&nbsp;&nbsp;7.2、房屋租金按<u> ${aesc.payment1} </u>支付。乙方应自本合同签订当日向甲方支付<u> ${aesc.payment2} </u>的租金，以后乙方应在<u> ${aesc.payment3} </u>的租金。乙方逾期交纳租金，逾期在15内的，每逾期一日，应按未交纳租金的总额0.5％的标准向甲方支付滞纳金；逾期时间超过15日的，甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
				<p>&nbsp;&nbsp;7.3、前述租金的付款方式：现金或转账。</p>
				<p>&nbsp;&nbsp;7.4、上述收费起算时间：从<u> ${aesc.charge_year1} </u>年<u> ${aesc.charge_month1} </u>月<u> ${aesc.charge_day1} </u>日起算。</p>
				<p>&nbsp;&nbsp;7.5、此合同到期后，甲方有权对该场地的租金价格保持一定幅度的递增。原则上递增幅度将不超过10％。</p>
				...
				<p>&nbsp;&nbsp;9.5、前述物业费用按<u> ${aesc.payment5} </u>支付，在签定合同时，乙方应向物业公司支付<u> ${aesc.payment6} </u>的物业费，以后乙方应在<u> ${aesc.payment7} </u>的物业费。 </p>
				...
				<p>&nbsp;&nbsp;28、乙方不得在展厅内进行除车辆销售以外的活动，包括但不限于汽车装饰安装、贴膜、更换汽车配件等。</p>
				...
				<p>&nbsp;&nbsp;37、在合同期内，乙方因自身原因需提前解除合同，须提前三十日以书面形式向甲方提出，经甲方书面同意方可办理解除租赁手续；未经甲方书面同意，乙方单方解除行为无效。且乙方所缴纳的剩余租金和履约保证金将作为违约金支付给甲方。</p>

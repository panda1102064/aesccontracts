...
<body>
	...
	<div id="wrap">
            <form action="updateMcgctcc" method="post">
                ...
                <p>&nbsp;&nbsp;1、房屋（场地）租金（按建筑面积计收）为每月租金合计人民币：<input required="required" class="dtkzNum" name="rent1" value="${aesc.rent1}" />(大写)<input required="required" class="dtkz" name="upper2" value="${aesc.upper2}" />。</p>
                <p>&nbsp;&nbsp;2、 乙方应向甲方支付履约保证金：￥<input required="required" class="dtkzNum" name="cash_deposit" value="${aesc.cash_deposit}" />，大写：<input required="required" class="dtkz" name="upper3"  value="${aesc.upper3}" />。在签定合同后由乙方一次性付清。合同有效期内若乙方擅自解除合同或有其它违反合同行为的，履约保证金和所剩租金不予退还，并纳入违约金范围之内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，只退本不退息。</p>
                <p>&nbsp;&nbsp;3、租金为<input required="required" class="dtkz" name="payment1" value="${aesc.payment1}" />支付，在签定合同时，乙方应向甲方支付<input required="required" class="dtkz" name="payment2" value="${aesc.payment2}" />的租金，以后乙方应在<input required="required" class="dtkz" name="payment3" value="${aesc.payment3}" />10日前应<input required="required" class="dtkz" name="payment4" value="${aesc.payment4}" />的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过10日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
                <p>&nbsp;&nbsp;4、付款方式：现金或转帐支票。</p>
                <p>第二十八条&nbsp;上述收费起算时间：双方约定起租时间从<input required="required" class="dtkzNum" name="lease_year" value="${aesc.lease_year}" />年<input required="required" class="dtkzNum" name="lease_month" value="${aesc.lease_month}" />月<input required="required" class="dtkzNum" name="lease_day" value="${aesc.lease_day}" />日起算。</p>

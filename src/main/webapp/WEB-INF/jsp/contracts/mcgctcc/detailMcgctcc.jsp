...
<body>
	<div id="wrap">
	<div class="PageNext"><jsp:include page="../../cover.jsp"></jsp:include></div>
                ...
                <p>第十五条&nbsp;甲方不得任意安排车辆到停车场停发，如遇特殊情况需按每车/每天15元缴纳费用。</p>
                ...
                <p>&nbsp;&nbsp;1、房屋（场地）租金（按建筑面积计收）为每月租金合计人民币：<u> ${aesc.rent1} </u>(大写)<u> ${aesc.upper2} </u>。</p>
                <p>&nbsp;&nbsp;2、 乙方应向甲方支付履约保证金：￥<u> ${aesc.cash_deposit} </u>，大写：<u> ${aesc.upper3} </u>。在签定合同后由乙方一次性付清。合同有效期内若乙方擅自解除合同或有其它违反合同行为的，履约保证金和所剩租金不予退还，并纳入违约金范围之内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，只退本不退息。</p>
                <p>&nbsp;&nbsp;3、租金为<u> ${aesc.payment1} </u>支付，在签定合同时，乙方应向甲方支付<u> ${aesc.payment2} </u>的租金，以后乙方应在<u> ${aesc.payment3} </u>10日前应<u> ${aesc.payment4} </u>的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过10日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
                <p>&nbsp;&nbsp;4、付款方式：现金或转账。</p>
                <p>第二十八条&nbsp;上述收费起算时间：双方约定起租时间从<u> ${aesc.lease_year} </u>年<u> ${aesc.lease_month} </u>月<u> ${aesc.lease_day} </u>日起算。</p>

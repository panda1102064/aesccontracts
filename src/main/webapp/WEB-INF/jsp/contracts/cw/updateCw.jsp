...
<%-- <jsp:include page="../../headpart.jsp"></jsp:include> --%>
	<div id="wrap">
		<form action="updateCw" method="post">
			...
			<p>地址：<input  class="dtkz" name="address" value="${aesc.address}" /></p>
			<p>根据《中华人民共和国合同法》及有关法律法规的规定，本着平等自愿和互惠互利的原则，为了明确甲乙双方的权利义务关系，就乙方需租赁甲方<input required="required" class="dtkz" name="area" value="${aesc.area}" />车位协商一致后，达成如下协议：</p>
			<p>一、租赁物业基本情况</p>
			<p>&nbsp;&nbsp;1、乙方租用甲方<input required="required" class="dtkzNum" name="carport_number" value="${aesc.carport_number}" />个车位，编号为<input required="required" class="dtkzNum" name="carport_serial" value="${aesc.carport_serial}" />号，单价：￥<input required="required" class="dtkzNum" name="price" value="${aesc.price}" />元/个/月（人民币），总计：￥<input required="required" class="dtkzNum" name="total" value="${aesc.total}" />元/月（人民币）；</p>
			<p>二、租赁期限</p>
			<p>&nbsp;&nbsp;上述车位租赁期限原则上为<input required="required" class="dtkzNum" name="times" value="${aesc.times}" />年，从<input required="required" class="dtkzNum" name="year_start" value="${aesc.year_start}" />年<input required="required" class="dtkzNum" name="month_start" value="${aesc.month_start}" />月<input required="required" class="dtkzNum" name="day_start" value="${aesc.day_start}" />日起至<input required="required" class="dtkzNum" name="year_end" value="${aesc.year_end}" />年<input required="required" class="dtkzNum" name="month_end" value="${aesc.month_end}" />月<input required="required" class="dtkzNum" name="day_end" value="${aesc.day_end}" />日止。</p>
			<p>三、租金及交付方式</p>
			<p>&nbsp;&nbsp;1、租金为每<input required="required" class="dtkz" name="pay_one" value="${aesc.pay_one}" />支付，在签定合同时，首次支付<input required="required" class="dtkz" name="pay_two" value="${aesc.pay_two}" />的租金。乙方应向甲方支付履约保证金：￥<input required="required" class="dtkz" name="cash_deposit" value="${aesc.cash_deposit}" />元，乙方应向甲方支付第一个<input required="required" class="dtkz" name="rent_one" value="${aesc.rent_one}" />的租金，以后乙方应在每<input required="required" class="dtkz" name="rent_two" value="${aesc.rent_two}" />支付下个<input required="required" class="dtkz" name="rent_three" value="${aesc.rent_three}" />的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过15日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
			...
			<p>四、乙方租用甲方<input required="required" class="dtkz" name="area_four" value="${aesc.area_four}" />用于<input required="required" class="dtkz" name="purpose" value="${aesc.purpose}" />车辆停放。乙方租赁的上述场地，仅供自己使用，不得擅自转租、转让、许可第三方使用或擅自改变合同所有约定的停放车型。如甲方发现乙方有上诉违约行为，甲方有权单方面解除合同，且不退还乙方已缴纳的租金。</p>
			...
			<p>&nbsp;&nbsp;c）本合同附件有：<input class="dtkz" name="accessory"  value="${aesc.accessory}" /></p>
			<p>十四、备注及补充说明</p>
			...
			<p>&nbsp;&nbsp;2.<input class="dtkz" name="remark" value="${aesc.remark}" /></p>
			<div id="me">
				<p>甲方签章：重庆汽博实业有限公司</p>

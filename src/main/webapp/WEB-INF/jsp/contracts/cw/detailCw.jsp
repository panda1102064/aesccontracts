...
<body>
	<div id="wrap">
			...
			<p>地址：<u> ${aesc.address}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </u></p>
			<p>根据《中华人民共和国合同法》及有关法律法规的规定，本着平等自愿和互惠互利的原则，为了明确甲乙双方的权利义务关系，就乙方需租赁甲方<u> ${aesc.area} </u>车位协商一致后，达成如下协议：</p>
			<p>一、租赁物业基本情况</p>
			<p>&nbsp;&nbsp;1、乙方租用甲方<u> ${aesc.carport_number} </u>个车位，编号为<u> ${aesc.carport_serial} </u>号，单价：￥<u> ${aesc.price} </u>元/个/月（人民币），总计：￥<u> ${aesc.total} </u>元/月（人民币）；</p>
			<p>二、租赁期限</p>
			<p>&nbsp;&nbsp;上述车位租赁期限原则上为<u> ${aesc.times} </u>年，从<u> ${aesc.year_start} </u>年<u> ${aesc.month_start} </u>月<u> ${aesc.day_start} </u>日起至<u> ${aesc.year_end} </u>年<u> ${aesc.month_end} </u>月<u> ${aesc.day_end} </u>日止。</p>
			<p>三、租金及交付方式</p>
			<p>&nbsp;&nbsp;1、租金为每<u> ${aesc.pay_one} </u>支付，在签定合同时，首次支付<u> ${aesc.pay_two} </u>的租金。乙方应向甲方支付履约保证金：￥<u> ${aesc.cash_deposit} </u>元，乙方应向甲方支付第一个<u> ${aesc.rent_one} </u>的租金，以后乙方应在每<u> ${aesc.rent_two} </u>支付下个<u> ${aesc.rent_three} </u>的租金。乙方逾期交纳租金，每逾期一日，应按未交纳租金总额的0.5％向甲方支付滞纳金。逾期时间不得超过15日，否则甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
			<p>&nbsp;&nbsp;2、合同有效期内若乙方擅自解除合同或有其他违反合同行为的，甲方有权力单方面终止合同，履约保证金和所剩租金不予退还，并纳入违约金范围内。违约金不足以弥补损失的，另行赔偿。合同到期后，如乙方无违约行为，履约保证金如数退还，只退本不退息。</p>
			<p>&nbsp;&nbsp;3、付款方式：现金或转帐。</p>
			<p>&nbsp;&nbsp;4、开户银行：建行重庆两江汽博支行     账号：5000 1040 0430 5250 0282</p>
			<p>四、乙方租用甲方<u> ${aesc.area_four} </u>用于<u> ${aesc.purpose} </u>车辆停放。乙方租赁的上述场地，仅供自己使用，不得擅自转租、转让、许可第三方使用或擅自改变合同所有约定的停放车型。如甲方发现乙方有上诉违约行为，甲方有权单方面解除合同，且不退还乙方已缴纳的租金。</p>
			...
			<p>&nbsp;&nbsp;c）本合同附件有：<u> ${aesc.accessory} </u></p>
			<p>十四、备注及补充说明</p>

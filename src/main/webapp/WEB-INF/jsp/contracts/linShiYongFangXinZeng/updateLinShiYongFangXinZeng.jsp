...
<body>
		<div id="wrap">
			<form action="updateLsyf_xz" method="post">
				...
				<p>&nbsp;&nbsp;2、乙方作为入驻汽博中心<input required="required" class="dtkz" name="area3" value="${aesc.area3}" />的承租人，承诺自愿接受由甲方委托的重庆博展物业管理有限公司（以下简称“物业公司”）担任在汽博中心的物业服务企业，并按照本合同的约定向物业公司履行相关义务（包括但不限于以物业使用人的身份缴纳物管费、水电费等）。在租赁期间，若物业公司不再担任汽博中心的物业服务企业，则乙方应当向实际担任汽博中心的物业服务企业缴纳相关费用。</p>
				<p class="zc">第二章     租赁物业概况</p>
        		<p>&nbsp;&nbsp;3、租赁物业基本情况：</p>
				<p>&nbsp;&nbsp;3.1、乙方承租的甲方商业用房（场地），位于：重庆市北部新区经开园金渝大道99号（以下简称“汽博中心”）内的<input required="required" class="dtkz" name="rental_location"value="${aesc.rental_location}" />，其建筑面积为：<input required="required" class="dtkzNum" name="building_area" value="${aesc.building_area}"/> ㎡ 。该租赁场地的房屋（场地）租金、物业管理服务费或其它根据场地面积计算的款项均以本款约定的租赁建筑面积计算。</p>
				<p>&nbsp;&nbsp;3.2、租赁物业的前上述信息与《房屋产权登记证》所载明的信息不一致时，以该登记为准。</p>
		        <p class="zc">第三章     租赁期限及租赁物业交付</p>
		        <p>&nbsp;&nbsp;4、本合同项下商业用房（场地）租赁期限为<input required="required" class="dtkzNum" name="expires" value="${aesc.expires}"/>年，即从自<input required="required" class="dtkzNum" name="year_start" id="year_start"value="${aesc.year_start}" />年<input required="required" class="dtkzNum" name="month_start" value="${aesc.month_start}" />月<input required="required" class="dtkzNum" name="day_start" value="${aesc.day_start}" />日起至<input required="required" class="dtkzNum" name="year_end" id="year_end" value="${aesc.year_end}"/>年<input required="required" class="dtkzNum" name="month_end" value="${aesc.month_end}" />月<input required="required" class="dtkzNum" name="day_end" value="${aesc.day_end}" />日止。</p>
		        <p>&nbsp;&nbsp;5、甲方于<input required="required" class="dtkzNum" name="transfer_year" id="transfer_year"value="${aesc.transfer_year}"/>年<input required="required" class="dtkzNum" name="transfer_month" value="${aesc.transfer_month}" />月<input required="required" class="dtkzNum" name="transfer_day" value="${aesc.transfer_day}"/>日前将本合同项下的商业用房（场地）移交给乙方使用。</p>
		        <p>&nbsp;&nbsp;6、交接房时，乙方须对租赁场房屋（场地）内已有的物品及设施逐一进行检查，经双方书面确认物品及设施完好后，甲方交付乙方使用；在合同有效期内，乙方应确保其租赁场地内的物品及设施的完好，如在使用中出现的损耗（如玻璃幕墙破裂，卷帘门损坏，门、窗、门锁损坏等），由乙方负责维修，如出现不能维修的情况，由乙方负责照价赔偿或自行更换。合同期满后，乙方应保证租赁场地内的物品及设施完好无损交付甲方。</p>
		        <p class="zc">第四章   乙方应支付费用</p>
		        <p>&nbsp;&nbsp;7、乙方应当向甲方缴纳的房屋租赁费用：</p>
		        <p>&nbsp;&nbsp;7.1、房屋（场地）租金（按乙方实际承租的建筑面积计收）为：</p>
            	<div id="main">
					<ol>
						<c:forEach items="${requestScope.lsyfList}" var="lsyfList">
							<li><p><input type="hidden" name="date_id" value="${lsyfList.date_id}" /><input required="required" class="dtkzNum" name="year_str" value="${lsyfList.year_str}" />年<input required="required" class="dtkzNum" name="month_str" value="${lsyfList.month_str}" />月<input required="required" class="dtkzNum" name="day_str" value="${lsyfList.day_str}" />日至<input required="required" class="dtkzNum" name="year_ed" value="${lsyfList.year_ed}" />年<input required="required" class="dtkzNum" name="month_ed" value="${lsyfList.month_ed}" />月<input required="required" class="dtkzNum" name="day_ed" value="${lsyfList.day_ed}" />日，每月租金合计为：<input required="required" class="dtkzNum" name="price" value="${lsyfList.price}" />元，大写：<input required="required" class="dtkzNum" name="upper" value="${lsyfList.upper}" />元整；</p><!-- <input class="btn-danger" type="button" value="添加" id="bt" /> --></li>
						</c:forEach>
					</ol>
				</div>
		        <p>&nbsp;&nbsp;7.2、房屋租金按<input required="required" class="dtkzNum" name="payment1" value="${aesc.payment1}"/>支付。在签定合同时，乙方应向甲方支付<input required="required" class="dtkzNum" name="payment2" value="${aesc.payment2}"/>的租金，以后乙方应在<input required="required" class="dtkzNum" name="payment3" value="${aesc.payment3}"/>的租金。乙方逾期交纳租金，逾期在15内的，每逾期一日，应按未交纳租金的总额0.5％的标准向甲方支付滞纳金；逾期时间超过15日的，甲方有权单方解除合同，不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。</p>
		        <p>&nbsp;&nbsp;7.3、前述租金的付款方式：现金或转帐。</p>
		        <p>&nbsp;&nbsp;7.4、上述收费起算时间：从<input required="required" class="dtkzNum" name="charge_year1" id="charge_year1" value="${aesc.charge_year1}"/>年<input required="required" class="dtkzNum" name="charge_month1" value="${aesc.charge_month1}"/>月<input required="required" class="dtkzNum" name="charge_day1" value="${aesc.charge_day1}" />日起算。</p>
		        ...
		        <p>&nbsp;&nbsp;9.1、物业管理服务费（按乙方承租的物业实际建筑面积计收），汽博中心<input required="required" class="dtkz" name="area4" value="${aesc.area4}" />的用房为商业性质的物业，物业管理服务费的收费标准经重庆市物局审批为：<input required="required" class="dtkzNum" name="management_fee1" value="${aesc.management_fee1}"/>元/㎡/月，物业管理服务费：<input required="required" class="dtkzNum" name="management_fee2" value="${aesc.management_fee2}"/>元/月。</p>
		        <p>&nbsp;&nbsp;9.2、中央空调使用耗能费：按实际公示标准收费。</p>
		        <p>&nbsp;&nbsp;9.3、商业用户水电交费单价分别为：电<input required="required" class="dtkzNum" name="power_rate" value="${aesc.power_rate}" />元人民币/KW/小时，水<input required="required" class="dtkzNum" name="water_rate" value="${aesc.water_rate}"/>元/T（含公共区域分摊费）。</p>
		        <p>&nbsp;&nbsp;9.4、公共区域的水电等公摊费用按据实分摊的原则收取。</p>
		        <p>&nbsp;&nbsp;9.5、前述物业费用按季度支付：在签定合同时，乙方应向甲方支付<input required="required" class="dtkz" name="payment4" value="${aesc.payment4}" />的物业费，以后乙方应在<input required="required" class="dtkz" name="payment5" value="${aesc.payment5}" />的物业费。</p>
		        ...
		        <p>&nbsp;&nbsp;10.1、上述收费起算时间：从<input required="required" class="dtkzNum" name="charge_year2" id="charge_year2" value="${aesc.charge_year2}"/>年<input required="required" class="dtkzNum" name="charge_month2" value="${aesc.charge_month2}"/>月<input required="required" class="dtkzNum" name="charge_day2" value="${aesc.charge_day2}" />日起算。</p>
		        ...
		        <p>&nbsp;&nbsp;17、汽博中心是一个开放式的商业园区，在相应的区域均设有临时停车位，供车辆使用人随机停放，计时收取场地占用费。未经许可，任何人不得将车辆停放于其他物业使用人的门店前已租赁的停车位及公共通道，如接到物业使用人受阻碍的投诉或巡查安保发现占用公共通道，甲方有权对乱停放车辆予以锁闭。</p>
		        ...
		        <p>&nbsp;&nbsp;29、乙方如需在物业公共及公用部位接搭电源时，必须提前告知甲方并经同意后在甲方指导下实施。</p>
		        <p>&nbsp;&nbsp;30、乙方专用区域内的消防安全责任由乙方自行负责，自觉遵守《中华人民共和国消防法》之规定。</p>
		        <p>&nbsp;&nbsp;31、乙方充分享有相关法律、法规规定的其他权利，同时也应承担相关法律、法规规定的义务。</p>
		        <p>&nbsp;&nbsp;32、乙方在合同期限届满或因故提前终止时，乙方应在撤离该场地前，须将该租赁房屋（场地）恢复到甲方交付该租赁房屋（场地）的时原貌，或保持为乙方向甲方申请的装修原装，如果乙方不能及时的恢复该租赁房屋（场地）时，甲方有权不予退还乙方的履约保证金和所剩租金，维修费不足以弥补损失的，另行赔偿。</p>
		        <p>&nbsp;&nbsp;33、乙方在合同期限届满或因故提前终止时，乙方应及时向甲方办理交接手续，每逾期一日办理交接手续，乙方应按当月月租金的5％的金额向甲方支付滞纳金。逾期时间不得超过3日，否则甲方有权不予退还履约保证金，并可向乙方追收所欠租金及滞纳金。若乙方未按照前述约定向甲方交还租赁房屋（场地）的，甲方有权自行接管及进入租赁房屋（场地），乙方遗留在租赁房屋（场地）物品、设施，甲方亦有权自行处置，不对乙方承担任何赔偿责任，乙方对此无异议。</p>
		        ...
		        <p>&nbsp;&nbsp;36、在合同期内，乙方因自身原因需提前解除合同，须提前三十日以书面形式向甲方提出，经甲方书面同意方可办理解除租赁手续；未经甲方书面同意，乙方单方解除行为无效。且乙方所缴纳的剩余租金和履约保证金将作为违约金支付给甲方。</p>
		        ...
		        <p>&nbsp;&nbsp;42.2、因受送达人自己提供或者确认的送达地址不准确、拒不提供送达地址、送达地址变更未及时告知人民法院、受送达本人或者指定的代收人拒绝签收，导致诉讼文书未能被受送达人实</p>
		        <p>&nbsp;&nbsp;43、备注：<input name="remark" value="${aesc.remark}"></input></p>
